...
\begin{abstract}
  ...
  (\texttt{classify}) and adaptive, budget-controlled constrained generation
  (\texttt{generate})---both yielding a full probability distribution and a
  confidence score, and both available across four inference
  ...
  ports in R (\texttt{rollama-classifier}) and Rust
...
through an OpenAI-compatible endpoint. Unlike \texttt{ollama-classifier},
scikit-llm is a label-only classifier: it returns the predicted class but no
...
\begin{table}[htbp]
  \centering
  \caption{Core methods of the \texttt{ollama-classifier} API. Both
           \texttt{generate} and \texttt{classify} return a
  ...
  \begin{tabular}{lp{8.2cm}}
    ...
    \midrule
    \texttt{generate()} & Adaptive trie-masked constrained generation.
                          Reconstructs per-label log-probabilities from one
                          to \texttt{max\_calls} calls; budget-controlled,
                          may be approximate under partial coverage. \\
...
\paragraph{Adaptive trie-masked generation (\texttt{generate}).}
Candidate labels are tokenized into a prefix trie, and each constrained call
is restricted to trie tokens while returning the \texttt{top\_logprobs} of
\emph{every} sibling alternative, so labels the model did not emit are still
scored. A breadth-first search over clusters of labels sharing a scored
prefix spends between one and \texttt{max\_calls} calls. For each label
$y_i$ relative to the generated (``winning'') path $y^\ast$, the divergence
point
\[
...
excluded beyond it. Partial scores feed the same geometric-mean and softmax
as \texttt{classify}. The reported \texttt{coverage}$(y_i)$ is the fraction
of the label's tokens that were scored, and \texttt{approximate} is
\texttt{True} whenever any label has coverage below one. Setting
\texttt{max\_calls}\,=\,1 yields a single fast, approximate call; larger
budgets resolve more clusters adaptively; and \texttt{max\_calls}\,=\,\texttt{None}
exhausts the frontier to produce exact scores (degenerating to one call per
...
\begin{table}[htbp]
  \centering
  \caption{Backend differentiation in \texttt{ollama-classifier} (v0.5.0).
           All backends expose per-token log-probabilities and support both
...
\begin{lstlisting}[caption={Zero-shot classification with
  \texttt{ollama-classifier} (v0.5.0 API).}, label={lst:usage}]
...
\paragraph{R: \texttt{rollama-classifier}.}
\texttt{rollama-classifier} \citep{rollama-classifier} is an R package
(\texttt{library(rollama)}) that ports the full API to R, including the four
backends, both \texttt{classify} and \texttt{generate}, their batch twins, and
...
\begin{lstlisting}[language=R, caption={Minimal classification call
  in R with \texttt{rollama-classifier}.}, label={lst:r-port}]
library(rollama)
...
struct, and sync/async and batch variants---and is tracking Python v0.4.1.
It is added to a project with \texttt{ollama-classifier-rs = "0.4"} in
\texttt{Cargo.toml} \citep{rust}; a minimal call is
...
the above''). We run 24 variations: two BART and two scikit-llm
configurations, four \texttt{ollama-classifier} \texttt{classify}
configurations (names only, $+$opt-out, names$+$descriptions,
descriptions$+$opt-out), and the same four configurations under
\texttt{generate} at four call budgets $\texttt{max\_calls}\in\{1,3,5,8\}$.
...
\begin{table}[htbp]
  ...
  \caption{Zero-shot classification on COICOP Division~01.2 (637 products,
           7 subclasses). BART uses NLI zero-shot classification
           \citep{lewis2019bart}; Ollama \texttt{classify} uses multi-call
           completion scoring and \texttt{generate} uses adaptive
           trie-masked generation, both with Qwen2.5 3B-Instruct
           ...
           (partially covered) predictions for the \texttt{generate}
           strategy. \dm\ denotes not applicable.}
  \label{tab:benchmark}
  \resizebox{\textwidth}{!}{%
  \begin{tabular}{lcccccccc}
    ...
    & (\%) & (macro) & (macro) & (macro) & (\%) & (s) & & \\
    \midrule
    BART (names only) & 40.8 & 0.548 & 0.485 & 0.445 & \dm & 444.4 & \dm & \dm \\
    BART (names + opt-out) & 39.4 & 0.549 & 0.495 & 0.454 & 94.2 & 496.2 & \dm & \dm \\
    \addlinespace
    Ollama \texttt{classify} (names only) & 75.5 & 0.717 & 0.711 & 0.703 & \dm & 2965.0 & 4459 & 0 \\
    Ollama \texttt{classify} (names + opt-out) & 74.7 & 0.710 & 0.721 & 0.705 & 97.6 & 3282.5 & 5096 & 0 \\
    Ollama \texttt{classify} (names + desc.) & 74.4 & 0.708 & 0.745 & 0.712 & \dm & 3007.2 & 4459 & 0 \\
    Ollama \texttt{classify} (desc. + opt-out) & 71.3 & 0.708 & 0.733 & 0.709 & 94.5 & 3397.4 & 5096 & 0 \\
    \addlinespace
    scikit-llm (names only) & 73.5 & 0.690 & 0.689 & 0.667 & \dm & 176.0 & \dm & \dm \\
    scikit-llm (names + opt-out) & 68.0 & 0.631 & 0.701 & 0.651 & 89.6 & 182.7 & \dm & \dm \\
    \addlinespace
    Ollama \texttt{generate}, names only (mc=1) & 73.8 & 0.679 & 0.692 & 0.669 & \dm & 465.0 & 637 & 637 \\
    Ollama \texttt{generate}, names only (mc=3) & 62.8 & 0.605 & 0.589 & 0.581 & \dm & 1428.2 & 1911 & 637 \\
    Ollama \texttt{generate}, names only (mc=5) & 56.2 & 0.527 & 0.539 & 0.518 & \dm & 2246.0 & 3185 & 637 \\
    Ollama \texttt{generate}, names only (mc=8) & 50.9 & 0.502 & 0.511 & 0.502 & \dm & 2724.8 & 3886 & 0 \\
    Ollama \texttt{generate}, names + opt-out (mc=1) & 70.6 & 0.651 & 0.684 & 0.649 & 95.1 & 449.5 & 637 & 637 \\
    Ollama \texttt{generate}, names + opt-out (mc=3) & 61.9 & 0.597 & 0.598 & 0.573 & 96.2 & 1410.7 & 1911 & 637 \\
    Ollama \texttt{generate}, names + opt-out (mc=5) & 52.7 & 0.504 & 0.534 & 0.488 & 97.0 & 2222.9 & 3185 & 637 \\
    Ollama \texttt{generate}, names + opt-out (mc=8) & 44.3 & 0.463 & 0.497 & 0.470 & 92.8 & 3106.9 & 4517 & 0 \\
    Ollama \texttt{generate}, names + desc. (mc=1) & 72.8 & 0.700 & 0.727 & 0.697 & \dm & 453.9 & 637 & 637 \\
    Ollama \texttt{generate}, names + desc. (mc=3) & 65.0 & 0.640 & 0.666 & 0.635 & \dm & 1439.8 & 1911 & 637 \\
    Ollama \texttt{generate}, names + desc. (mc=5) & 61.7 & 0.582 & 0.639 & 0.583 & \dm & 2298.2 & 3185 & 637 \\
    Ollama \texttt{generate}, names + desc. (mc=8) & 61.4 & 0.575 & 0.633 & 0.579 & \dm & 2789.5 & 3894 & 0 \\
    Ollama \texttt{generate}, desc. + opt-out (mc=1) & 67.8 & 0.693 & 0.712 & 0.677 & 94.5 & 445.9 & 637 & 637 \\
    Ollama \texttt{generate}, desc. + opt-out (mc=3) & 61.5 & 0.641 & 0.644 & 0.602 & 96.7 & 1422.2 & 1911 & 637 \\
    Ollama \texttt{generate}, desc. + opt-out (mc=5) & 59.5 & 0.587 & 0.618 & 0.555 & 97.2 & 2283.9 & 3185 & 637 \\
    Ollama \texttt{generate}, desc. + opt-out (mc=8) & 57.5 & 0.540 & 0.605 & 0.550 & 96.5 & 3141.3 & 4515 & 0 \\
    \bottomrule
  \end{tabular}}
\end{table}
...
scikit-llm is also the fastest classifier ($\approx$176--183\,s), versus
$\approx$2{,}965--3{,}397\,s for multi-call \texttt{classify} and
$\approx$444--496\,s for BART on CPU. The \texttt{generate} strategy trades
accuracy for cost control: at \texttt{max\_calls}\,=\,1 it matches scikit-llm
(73.8\%, 637 calls), but accuracy falls monotonically as the budget grows,
because higher call budgets expose the strategy to its approximate regime---at
\texttt{max\_calls}\,$\leq$\,5 all 637 predictions are flagged approximate
(partial token coverage), and only at \texttt{max\_calls}\,=\,8 does coverage
complete (zero approximate items), yet accuracy is then lowest
(Figure~\ref{fig:generate}). The ``names + descriptions'' configuration decays
most gracefully and remains the strongest generate configuration at
\texttt{max\_calls}\,=\,8 (61.4\%).
...
Because scikit-llm is label-only, the confidence analysis covers the 22
confidence-bearing variations (BART, \texttt{classify}, \texttt{generate}).
...
and the point-biserial correlations are positive throughout ($r$ between 0.30
and 0.54). The two generative strategies are miscalibrated, but in opposite
...
\texttt{generate} is \emph{overconfident}: at low call budgets the mean
confidence on incorrect predictions (0.82--0.85) exceeds BART's mean
confidence on \emph{correct} predictions, and its curve bends below the
diagonal. BART is well calibrated (ECE$\approx$0.03--0.06), close to the
diagonal. Both generative strategies carry a large ECE
(\texttt{classify} $\approx$0.15--0.20; \texttt{generate}
$\approx$0.17--0.22), so their absolute probabilities are unreliable and call
...
confidence is 0.70 for BART but 0.79--0.83 for \texttt{classify}, indicating
that even though the absolute confidence values are poorly calibrated, they
order predictions by likelihood of correctness substantially better than
chance and better than BART.
...
\begin{table}[htbp]
  \centering
  \caption{Confidence and calibration across the 22 confidence-bearing
           variations. ``Conf'' is the mean predicted-label confidence on
           ...
           scikit-llm is omitted (label-only).}
  ...
  \begin{tabular}{lccccccccccc}
    ...
    \addlinespace
    Ollama \texttt{generate} (names only, mc=1) & 0.975 & 0.844 & 0.434 & 0.952 & 0.730 & 8.58 & 8.64 & 0.203 & 0.831 & 0.829 & 0.202 \\
    Ollama \texttt{generate} (names only, mc=3) & 0.907 & 0.677 & 0.500 & 0.819 & 0.402 & 13.65 & 13.11 & 0.194 & 0.832 & 0.805 & 0.213 \\
    Ollama \texttt{generate} (names only, mc=5) & 0.847 & 0.620 & 0.460 & 0.703 & 0.311 & 13.02 & 12.10 & 0.186 & 0.800 & 0.741 & 0.229 \\
    Ollama \texttt{generate} (names only, mc=8) & 0.809 & 0.581 & 0.449 & 0.643 & 0.256 & 12.66 & 12.05 & 0.188 & 0.779 & 0.716 & 0.236 \\
    Ollama \texttt{generate} (names + opt-out, mc=1) & 0.977 & 0.849 & 0.426 & 0.957 & 0.741 & 7.98 & 8.01 & 0.202 & 0.818 & 0.816 & 0.200 \\
    Ollama \texttt{generate} (names + opt-out, mc=3) & 0.907 & 0.696 & 0.453 & 0.817 & 0.443 & 11.64 & 11.06 & 0.189 & 0.808 & 0.780 & 0.218 \\
    Ollama \texttt{generate} (names + opt-out, mc=5) & 0.840 & 0.629 & 0.423 & 0.690 & 0.327 & 11.57 & 10.75 & 0.200 & 0.773 & 0.715 & 0.245 \\
    Ollama \texttt{generate} (names + opt-out, mc=8) & 0.797 & 0.584 & 0.414 & 0.613 & 0.266 & 11.00 & 9.95 & 0.208 & 0.760 & 0.685 & 0.253 \\
    Ollama \texttt{generate} (names + desc., mc=1) & 0.955 & 0.831 & 0.378 & 0.921 & 0.716 & 8.10 & 8.08 & 0.193 & 0.783 & 0.782 & 0.207 \\
    Ollama \texttt{generate} (names + desc., mc=3) & 0.939 & 0.734 & 0.493 & 0.888 & 0.524 & 12.49 & 12.26 & 0.217 & 0.834 & 0.828 & 0.221 \\
    Ollama \texttt{generate} (names + desc., mc=5) & 0.894 & 0.693 & 0.436 & 0.803 & 0.446 & 11.72 & 11.39 & 0.202 & 0.794 & 0.777 & 0.231 \\
    Ollama \texttt{generate} (names + desc., mc=8) & 0.829 & 0.678 & 0.316 & 0.683 & 0.429 & 8.29 & 7.81 & 0.167 & 0.705 & 0.683 & 0.244 \\
    Ollama \texttt{generate} (desc. + opt-out, mc=1) & 0.961 & 0.823 & 0.420 & 0.933 & 0.703 & 8.92 & 9.00 & 0.205 & 0.822 & 0.821 & 0.210 \\
    Ollama \texttt{generate} (desc. + opt-out, mc=3) & 0.941 & 0.709 & 0.536 & 0.894 & 0.474 & 13.98 & 13.90 & 0.220 & 0.853 & 0.849 & 0.216 \\
    Ollama \texttt{generate} (desc. + opt-out, mc=5) & 0.875 & 0.663 & 0.440 & 0.768 & 0.393 & 11.92 & 11.51 & 0.181 & 0.794 & 0.768 & 0.224 \\
    Ollama \texttt{generate} (desc. + opt-out, mc=8) & 0.805 & 0.655 & 0.304 & 0.649 & 0.383 & 7.94 & 7.86 & 0.176 & 0.684 & 0.675 & 0.250 \\
    \bottomrule
  \end{tabular}}
\end{table}
...
(Figures~\ref{fig:conf-boxplots} and \ref{fig:gap-boxplots}): across all 22
...
\begin{table}[htbp]
  \centering
  \caption{Threshold discrimination for the six headline variations. AUC is
           the ROC-AUC of each score against correctness; the remaining
...
\begin{figure}[htbp]
  ...
  \caption{Confidence distributions for correct (green) and incorrect (red)
           predictions across all 22 confidence-bearing variations. Each panel
...
\begin{figure}[htbp]
  ...
  \caption{Score-gap (top minus second-highest class probability)
           distributions for correct and incorrect predictions across all 22
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{generate_sweep}
  \caption{Adaptive \texttt{generate} strategy: accuracy, mean confidence,
           ECE, and AUC against the call budget (\texttt{max\_calls}), by
           choice configuration. The vertical dotted line marks the boundary
           between the approximate regime (\texttt{max\_calls}\,$\leq$\,5,
           all predictions approximate) and the exact point
           (\texttt{max\_calls}\,=\,8).}
  \label{fig:generate}
\end{figure}
...
for exact, calibrated probabilities, whereas \texttt{generate} reconstructs
scores from a budget-controlled number of constrained calls and flags
approximate results through its coverage field---letting users choose speed
over exactness and inspect partial coverage when they do.
...
completion scoring and adaptive trie-masked generation---across four
