...
For each candidate label **yᵢ**, the backend returns per-token log-probabilities
of that label's tokens **Tᵢ** given the prompt:

**Label score** (geometric mean of per-token log-probs):

`s(y_i) = (1 / |T_i|) · Σ log p(t | x, prefix_t)`

**Calibrated distribution** (max-subtracted softmax):

`P(y_i | x) = exp(s(y_i)) / Σ_j exp(s(y_j))`

`confidence = max_i P(y_i | x)`
...
`ClassificationResult` struct, sync/async/batch). 
Added with `ollama-classifier-rs = "0.5"` in `Cargo.toml`.
...
**Future work:** temperature scaling, conformal prediction, broader RAC pipeline, collaborative web interface
...
- `ollama-classifier-rs`: `ollama-classifier-rs = "0.5"`

**Contact:** Luigi Palumbo (luigi.palumbo@bancaditalia.it) · Mengting Yu (mengting.yu@unitus.it)
